  ...
  return (
    ...
    >
      <div className="mx-auto max-w-2xl px-6 text-center">
        ...
        <p
          ref={textRef}
          className="mt-6 text-base leading-relaxed text-foreground/80 md:text-lg opacity-0"
        >
          For collaboration or inquiries, please reach out via email:
          <br />
          <a
            href={`mailto:${emailRecipient}`}
            className="text-primary font-semibold underline underline-offset-4 mt-2 inline-block hover:text-primary-hover transition-colors"
          >
            {emailRecipient}
          </a>
        </p>
        <div ref={iconsRef} className="mt-10 flex flex-wrap justify-center gap-4 opacity-0">
          {CONTACT_LINKS.map((link) => {
            ...
            return (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="inline-flex h-14 w-14 items-center justify-center rounded-full border border-primary/30 bg-primary/5 text-primary transition hover:-translate-y-1 hover:border-primary/70 hover:bg-primary/10 shadow-sm"
