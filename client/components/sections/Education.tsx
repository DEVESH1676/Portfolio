    ...
    <section id="education" className="bg-secondary/40 py-24 scroll-mt-24">
      <div className="mx-auto max-w-6xl px-6">
        ...
        <div className="relative ml-2 pl-8">
          {/* Vertical line */}
          <div className="absolute left-1/2 top-0 bottom-0 w-1 -translate-x-1/2 bg-gradient-to-b from-primary/40 to-primary/10" aria-hidden />

          {EDUCATION_TIMELINE.map((entry, index) => {
            const isCurrent = index === 0; // Assuming first entry is Ph.D., current
            return (
              <div key={entry.degree} className="relative pb-12 last:pb-0">
                {/* Dot */}
                <div className="absolute left-1/2 -translate-x-1/2 flex items-center justify-center top-6">
                  {isCurrent ? (
                    // Current (Ph.D.) hollow ring
                    <div className="h-6 w-6 rounded-full border-2 border-primary/70 bg-secondary/40" />
                  ) : (
                    // Past entries solid with ring
