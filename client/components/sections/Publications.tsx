...
export const PublicationsSection = () => {
  ...
  React.useEffect(() => {
    import("@/lib/anime").then((mod) => {
      ...
      if (introRef.current)
        animateEntrance(introRef.current, { translateY: 16, duration: 600, delay: 70 });
      if (featuredRef.current)
        animateEntrance(featuredRef.current, { translateY: 20, duration: 700, delay: 140 });
      if (additionalRef.current)
        animateEntrance(additionalRef.current, { translateY: 20, duration: 700, delay: 220 });
    });
  }, []);
    ...
    <section id="publications" className="bg-secondary/40 py-24 scroll-mt-24">
      <div className="mx-auto max-w-6xl px-6">
        <div className="mb-12 max-w-3xl">
          <h2 ref={headingRef} className="font-heading text-3xl font-semibold tracking-tight text-foreground md:text-4xl">
            Selected Publications
          </h2>
          <p ref={introRef} className="mt-4 text-base text-foreground/75 md:text-lg">
        ...
        <div className="grid gap-10 lg:grid-cols-[1.2fr_0.8fr]">
          <Card ref={featuredRef} className="border border-primary/20 bg-background/95 shadow-xl">
            <CardHeader>
              ...
              <p className="text-sm text-primary/80">{FEATURED_PUBLICATION.venue}</p>
            </CardHeader>
          ...
          <Card ref={additionalRef} className="border border-primary/10 bg-primary/5 p-6 shadow-lg">
            <h3 className="font-heading text-xl font-semibold text-primary">Additional Publications</h3>
            <ul className="mt-4 space-y-4 text-sm text-foreground/85">
              {ADDITIONAL_PUBLICATIONS.map((publication) => (
                ...
                >
                  <span className="block leading-snug">{publication.title}</span>
